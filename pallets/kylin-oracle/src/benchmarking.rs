...
benchmarks! {
    ...
    //     let a in 1 .. 100;
    //     let caller: T::AccountId = whitelisted_caller();
    //     let feed_name = str::from_utf8(b"test_feed").unwrap().as_bytes().to_vec();
    //     let data = b"{'benchmark':'this is data'}".to_vec();
    //     let _ = Pallet::<T>::write_data_onchain(RawOrigin::Signed(caller.clone()).into(), feed_name.clone(), data);
    //     let new_feed_name = str::from_utf8(b"custom_").unwrap().to_owned() + str::from_utf8(&feed_name).unwrap();
    // }: _(RawOrigin::Signed(caller.clone()), None, new_feed_name.as_bytes().to_vec())
