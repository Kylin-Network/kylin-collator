...
use kylin_oracle::{CreatorId, OracleKeyOf};
use kylin_primitives::collection::{Collection, CollectionInfo};
use kylin_primitives::nft::NftInfo;
use kylin_primitives::nft::{AccountIdOrCollectionNftTuple, Nft};
...
#[derive(Encode, Decode, Serialize, Deserialize, Debug, Clone, PartialEq, Eq, TypeInfo)]
pub struct MetaData {
    key: Vec<u8>,
    url: Vec<u8>,
    vpath: Vec<u8>,
...
pub mod pallet {
        ...
        <T as frame_system::Config>::AccountId: AsRef<[u8]>,
    {
        ...
        pub fn create_feed(
            ...
            let key_limit: OracleKeyOf<T> = key
                .clone()
                .try_into()
            ...
            let metadata: BoundedVec<u8, T::StringLimit> = meta_str
                .as_bytes()
                .to_vec()
                .try_into()
                .map_err(|_| Error::<T>::StorageOverflow)?;
            ...
            Self::deposit_event(Event::FeedCreated {
                owner: nft_owner,
                collection_id,
                nft_id,
                metadata: mdata,
        ...
        pub fn remove_feed(
            ...
            let mdata: MetaData =
                serde_json::from_slice(&nft.metadata).map_err(|_| Error::<T>::JsonError)?;

            let cid = CreatorId::AccountId(sender.clone());
            let key_limit: OracleKeyOf<T> = mdata
                .key
                .clone()
                .try_into()
                .map_err(|_| Error::<T>::StorageOverflow)?;
            ...
            Self::deposit_event(Event::FeedRemoved {
                owner: sender,
                nft_id,
            });
            Ok(())
