...
pub struct DevelopmentRuntimerExecutor;
pub struct KylinRuntimerExecutor;
...
impl sc_executor::NativeExecutionDispatch for KylinRuntimerExecutor {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		kylin_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		kylin_runtime::native_version()
	}
}
...
pub fn build_pichiu_import_queue(
	client: Arc<TFullClient<Block, pichiu_runtime::RuntimeApi, NativeElseWasmExecutor<PichiuRuntimerExecutor>>>,
	config: &Configuration,
	...
	sc_consensus::DefaultImportQueue<
		Block,
		TFullClient<Block, pichiu_runtime::RuntimeApi, NativeElseWasmExecutor<PichiuRuntimerExecutor>>,
	>,
	sc_service::Error,
> {
...
/// Start a pichiu parachain node.
pub async fn start_pichiu_node(
	parachain_config: Configuration,
	polkadot_config: Configuration,
	collator_options: CollatorOptions,
	id: ParaId,
	...
	start_node_impl::<pichiu_runtime::RuntimeApi, PichiuRuntimerExecutor, _, _, _>(
		parachain_config,
		polkadot_config,
		collator_options,
		id,
...
pub fn build_development_import_queue(
	client: Arc<TFullClient<Block, development_runtime::RuntimeApi, NativeElseWasmExecutor<DevelopmentRuntimerExecutor>>>,
	config: &Configuration,
	telemetry: Option<TelemetryHandle>,
	task_manager: &TaskManager,
) -> Result<
	sc_consensus::DefaultImportQueue<
		Block,
		TFullClient<Block, development_runtime::RuntimeApi, NativeElseWasmExecutor<DevelopmentRuntimerExecutor>>,
	>,
	sc_service::Error,
> {
	let slot_duration = cumulus_client_consensus_aura::slot_duration(&*client)?;
...
pub async fn start_development_node(
	...
	start_node_impl::<development_runtime::RuntimeApi, DevelopmentRuntimerExecutor, _, _, _>(
		parachain_config,
		polkadot_config,
		collator_options,
		id,
...
pub fn build_kylin_import_queue(
	client: Arc<TFullClient<Block, kylin_runtime::RuntimeApi, NativeElseWasmExecutor<KylinRuntimerExecutor>>>,
	config: &Configuration,
	telemetry: Option<TelemetryHandle>,
	task_manager: &TaskManager,
) -> Result<
	sc_consensus::DefaultImportQueue<
		Block,
		TFullClient<Block, kylin_runtime::RuntimeApi, NativeElseWasmExecutor<KylinRuntimerExecutor>>,
	>,
	sc_service::Error,
> {
	let slot_duration = cumulus_client_consensus_aura::slot_duration(&*client)?;
...
pub async fn start_kylin_node(
	...
	TaskManager,
	Arc<TFullClient<Block, kylin_runtime::RuntimeApi, NativeElseWasmExecutor<KylinRuntimerExecutor>>>,
)> {
	start_node_impl::<kylin_runtime::RuntimeApi, KylinRuntimerExecutor, _, _, _>(
		parachain_config,
		polkadot_config,
		collator_options,
		id,
		|_| Ok(RpcModule::new(())),
		build_kylin_import_queue,
		|client,
		 prometheus_registry,
		 telemetry,
		 task_manager,
